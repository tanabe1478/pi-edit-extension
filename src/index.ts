...
export default function taggedEditExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "read_hashline_legacy",
    label: "Read hashline legacy",
    description:
      "Read a text file using the original compact hashline anchors only: LINEhh|content. Kept for before/after benchmarking; read_hashline is safer by default.",
    parameters: Type.Object({
      path: Type.String({ description: "File path, relative to current working directory or absolute" }),
      offset: Type.Optional(Type.Number({ description: "1-indexed start line", minimum: 1 })),
      limit: Type.Optional(Type.Number({ description: "Maximum number of lines", minimum: 1 })),
    ...
    async execute(_toolCallId, params) {
      ...
      const fileText = await fs.readFile(p, "utf8");
      hashlineSnapshots.set(p, fileText);
      const result = formatHashline(fileText, { offset: params.offset, limit: params.limit, strictMode: "none" });
      await appendMetric(process.env[METRICS_ENV], {
        tool: "read_hashline_legacy",
        path: p,
        linesReturned: result.end >= result.start ? result.end - result.start + 1 : 0,
        resultChars: result.text.length,
        resultTokenEstimate: estimateTokensFromChars(result.text.length),
  ...
  pi.registerTool({
    ...
    description:
      "Apply the original compact hashline patch semantics without adaptive strict range requirements. Kept for before/after benchmarking; edit_hashline_patch is safer by default.",
    parameters: Type.Object({
      input: Type.String({ description: "Patch text. Sections start with @@ PATH. Payload lines start with ~ by default." }),
      payloadSep: Type.Optional(Type.String({ description: "Payload line separator; default ~" })),
    }),
    async execute(_toolCallId, params) {
      ...
      for (const section of sections) {
        ...
        let recovered = false;
        try {
          result = validateAndApplyHashlinePatch(before, section, { payloadSep: params.payloadSep ?? "~", requireStrictRanges: false });
        } catch (err) {
          if (!(err instanceof HashlineMismatchError) || !hashlineSnapshots.has(p)) throw err;
          result = recoverHashlinePatchFromSnapshot(hashlineSnapshots.get(p) as string, before, section, { payloadSep: params.payloadSep ?? "~", requireStrictRanges: false });
          recovered = true;
        }
        await fs.writeFile(p, result.text, "utf8");
        hashlineSnapshots.set(p, result.text);
      ...
      await appendMetric(process.env[METRICS_ENV], {
        tool: "edit_hashline_patch_legacy",
        sections: results.length,
        ops: results.reduce((sum, r) => sum + r.ops, 0),
        recovered: results.some((r) => r.recovered),
        inputChars: estimateJsonChars(params),
